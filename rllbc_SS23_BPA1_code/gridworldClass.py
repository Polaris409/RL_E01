...
class Gridworld(mdp.MarkovDecisionProcess):
    ...
    def getNorth(self, state):
        row, col = state
        if row == 0:
            return (row, col)  # If already at the top boundary, stay in the same state
        else:
            return (row - 1, col)

    def getSouth(self, state):
        row, col = state

    def getEast(self, state):
        row, col = state

    def getWest(self, state):
        row, col = state

    def getNextState(self, state, action):
        if action == 'north':
            return self.getNorth(state)
        elif action == 'south':
            return self.getSouth(state)
        elif action == 'east':
            return self.getEast(state)
        elif action == 'west':
            return self.getWest(state)
        else:
            return state  # If the action is invalid, stay in the same state
    ...
    def getTransitions(self, state, action):
        ...
        if action == 'north':
            transitions.append((self.getNorth(state), reward, 0.8))
            transitions.append((self.getEast(state), reward, 0.1))
            transitions.append((self.getWest(state), reward, 0.1))
        elif action == 'south':
            transitions.append((self.getSouth(state), reward, 0.8))
            transitions.append((self.getEast(state), reward, 0.1))
            transitions.append((self.getWest(state), reward, 0.1))
        elif action == 'east':
            transitions.append((self.getEast(state), reward, 0.8))
            transitions.append((self.getNorth(state), reward, 0.1))
            transitions.append((self.getSouth(state), reward, 0.1))
        elif action == 'west':
            transitions.append((self.getWest(state), reward, 0.8))
            transitions.append((self.getNorth(state), reward, 0.1))
            transitions.append((self.getSouth(state), reward, 0.1))
    ...
    def isTerminal(self, state):
        return state == (1, 0) or state == (1, 4) or state[0] == 2  # Check if the state is terminal
